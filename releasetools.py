...
#
# xda Bootloader packed image format
#
...
class xdaBootImage(object):
  ...
  def _unpack(self, data):
    """Unpack the data blob as a xda boot image and return the list
...
def IncrementalOTA_VerifyEnd(info):
  ...
  target_modem_img = xdaBootImage(target_radio_img).GetUnpackedImage("modem")
  if not target_modem_img: return
  source_modem_img = xdaBootImage(source_radio_img).GetUnpackedImage("modem")
...
def WriteIncrementalBootloader(info, target_imagefile, source_imagefile):
  try:
    tm = xdaBootImage(target_imagefile.data, "bootloader")
  except BadMagicError:
    raise ValueError("bootloader.img bad magic value")
  try:
    sm = xdaBootImage(source_imagefile.data, "bootloader")
  except BadMagicError:
    print "source bootloader is not a xda boot img; installing complete img."
...
def WriteIncrementalRadio(info, target_imagefile, source_imagefile):
  try:
    target_radio_img = xdaBootImage(target_imagefile.data, "radio")
  except BadMagicError:
    print "Magic number mismatch in target radio image"
    raise ValueError("radio.img bad magic value")

  try:
    source_radio_img = xdaBootImage(source_imagefile.data, "radio")
  except BadMagicError:
  ...
  WritexdaBootPartitionImages(info, target_radio_img, blacklist)
...
def WriteRadio(info, radio_img):
  ...
  try:
    xda_boot_image = xdaBootImage(radio_img, "radio")
  except BadMagicError:
    raise ValueError("radio.img bad magic value")

  WritexdaBootPartitionImages(info, xda_boot_image)


def WritexdaBootPartitionImages(info, xda_boot_image, blacklist=[]):
  WriteGroupedImages(info, xda_boot_image.name,
                     xda_boot_image.unpacked_images.values(), blacklist)
...
def WriteBootloader(info, bootloader,
  ...
  try:
    xda_boot_image = xdaBootImage(bootloader,"bootloader")
  except BadMagicError:
    raise ValueError("bootloader.img bad magic value")
  ...
  WritexdaBootPartitionImages(info, xda_boot_image, blacklist)
